...
long Read(pFF_p_mmap const &map, long const &offset, string * const &pt)
{
    if(pt == NULL) {
        libf_HandleError("String is empty! It needs to be initialized.", 319);
    }
    long len = 0;
    char *d = const_cast<char*>(pt->data());
    long l = ffmmap_read(map->map, &len, sizeof(long), offset);
    pt->resize(len);
    long s = ffmmap_read(map->map, d, len, offset+sizeof(long));
    return l+s;
}

long Write(pFF_p_mmap const &map, long const &offset, string *const &pt)
{
    if(pt == NULL) {
        libf_HandleError("String is empty! It needs to be initialized.", 319);
    }
    long len = pt->size();
    char *d = const_cast<char*>(pt->data());
    long l = ffmmap_write(map->map, &len, sizeof(long), offset);
    long s = ffmmap_write(map->map, d, len, offset+sizeof(long));
    return l+s;
}
...
template <class T>
long Read(pFF_p_mmap const &map, const long &offset, KNM<T> *const &data)
{
    long n = data->N(), m = data->M(), lnm = n * m * sizeof(T);
    T *pt = *data;
    return ffmmap_read(map->map, (void *)pt, lnm, offset);
}

template <class T>
long Write(pFF_p_mmap const &map, const long &offset, KNM<T> *const &data)
{
    long n = data->N(), m = data->M(), lnm = n * m * sizeof(T);
    T *pt = *data;
    return ffmmap_write(map->map, (void *)pt, lnm, offset);
}
...
static void init()
{
    ...
    Global.Add("Read",  "(",  new OneOperator3_<long, FF_p_mmap *, long, string *>(Read));
    Global.Add("Write", "(",  new OneOperator3_<long, FF_p_mmap *, long, string *>(Write));
    ...
    Global.Add("Read",  "(",  new OneOperator3_<long, FF_p_mmap *, long, KNM<long> *>(Read));
    Global.Add("Write", "(",  new OneOperator3_<long, FF_p_mmap *, long, KNM<long> *>(Write));
    ...
    Global.Add("Read",  "(",  new OneOperator3_<long, FF_p_mmap *, long, KNM<double> *>(Read));
    Global.Add("Write", "(",  new OneOperator3_<long, FF_p_mmap *, long, KNM<double> *>(Write));
    ...
    Global.Add("Read",  "(",  new OneOperator3_<long, FF_p_mmap *, long, KNM<Complex> *>(Read));
    Global.Add("Write", "(",  new OneOperator3_<long, FF_p_mmap *, long, KNM<Complex> *>(Write));
}
