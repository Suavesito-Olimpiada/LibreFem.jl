...
void ffmmap_init(ff_Pmmap p, const char *nm, long len)
{
    ...
    p->map = 0;
#if defined(_WIN32)
    p->fd = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0,
                              len, nm);
    if (p->fd == NULL) {
#else
...
#if defined(_WIN32)
    p->map = MapViewOfFile(p->fd, FILE_MAP_ALL_ACCESS, 0, 0, p->len);
    if (p->map == NULL) {
#else
    ...
    p->map = mmap(addr, p->len, PROT_READ | PROT_WRITE, MAP_FILE | MAP_SHARED,
                  p->fd, 0);
    if (p->map == MAP_FAILED) {
#endif
        p->map = 0;
...
long ffmmap_read(ff_Pmmap p, void *pt, size_t ln, long off)
{
    if (off < 0 || off + ln > p->len) {
        printf(
            "Fatal Error: ffmmap_read ff mmap out of bound len = %zu < %lu + "
            "%ld \n",
...
long ffmmap_write(ff_Pmmap p, void *pt, size_t ln, long off)
{
    if (off < 0 || off + ln > p->len) {
        printf(
            "Fatal Error: ffmmap_write ff mmap out of bound len = %zu < %lu + "
            "%ld \n",
            p->len, (unsigned long)ln, off);
