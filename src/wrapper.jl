module LibFFMmapSemaphore

const libff_mmap_semaphore = "../lib/libff-mmap-semaphore.so"
...
macro load!(symbol)
    :(
        $symbol.x = Core.Intrinsics.cglobal(($(esc(QuoteNode(symbol))), $(QuoteNode(libff_mmap_semaphore))), eltype($symbol));
        $symbol
    )
end

const ff_mmap_sem_verb = Global{Clong}()
@load! ff_mmap_sem_verb
